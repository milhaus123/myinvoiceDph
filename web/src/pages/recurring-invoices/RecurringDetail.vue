...
async function load() {
  ...
  try {
    ...
    const [tpl, runs, invoices] = await Promise.all([
      ...
      recurringInvoicesApi.nextRuns(),
      recurringInvoicesApi.generatedInvoices(id),
    ])
    template.value = tpl
...
async function pauseTemplate() {
  ...
  busy.value = 'pause'
  try {
    template.value = await recurringInvoicesApi.pause(template.value.id)
...
async function resumeTemplate() {
  ...
  busy.value = 'resume'
  try {
    template.value = await recurringInvoicesApi.resume(template.value.id)
...
async function runNow() {
  ...
  busy.value = 'run-now'
  try {
    const result = await recurringInvoicesApi.runNow(
      template.value.id,
      runNowDate.value || undefined
    )
    toast.success(t('recurring_invoice.run_now_success', { number: result.invoice_number }))
    runNowDate.value = ''
    await load()
  } catch (e: any) {
    toast.error(e?.response?.data?.error?.message || t('recurring_invoice.run_now_failed'))
  } finally {
    busy.value = null
  }
}
...
async function deleteTemplate() {
  ...
  try {
    await recurringInvoicesApi.delete(template.value.id)
  ...
  <div v-else>
    ...
    <div class="flex items-center justify-between mb-4">
        ...
        <h1 class="text-2xl font-semibold mt-1">
          ...
          <span class="ml-2 text-sm font-normal">
            <span class="px-2 py-0.5 rounded" :class="statusBadgeClass(template.status)">
              {{ t(`recurring_invoice.status.${template.status}`) }}
            </span>
          </span>
        </h1>
      </div>
      <div class="flex items-center gap-2 flex-wrap">
        <button
          ...
          {{ busy === 'pause' ? '…' : t('recurring_invoice.pause') }}
        </button>
        <button
          ...
          {{ busy === 'resume' ? '…' : t('recurring_invoice.resume') }}
        </button>
        <button
          v-if="template.status === 'active'"
          @click="runNow"
          ...
          {{ busy === 'run-now' ? '…' : t('recurring_invoice.run_now') }}
        </button>
        <RouterLink
          :to="`/recurring-invoices/${template.id}/edit`"
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
      ...
      <div class="bg-white border border-neutral-200 rounded-lg p-5 shadow-sm">
        <h3 class="text-sm font-semibold uppercase tracking-wide text-neutral-500 mb-3">{{ t('recurring_invoice.template_info') }}</h3>
        <dl class="space-y-1.5 text-sm">
          <div class="flex justify-between">
            <dt class="text-neutral-500">{{ t('recurring_invoice.client') }}</dt>
            <dd class="font-medium">{{ template.client_company_name }}</dd>
          </div>
          <div v-if="template.project_name" class="flex justify-between">
            <dt class="text-neutral-500">{{ t('recurring_invoice.project') }}</dt>
            <dd class="font-medium">{{ template.project_name }}</dd>
          </div>
          <div class="flex justify-between">
            <dt class="text-neutral-500">{{ t('recurring_invoice.frequency') }}</dt>
            <dd class="font-medium">{{ frequencyLabel(template.frequency) }}</dd>
          </div>
          <div class="flex justify-between">
            <dt class="text-neutral-500">{{ t('recurring_invoice.day') }}</dt>
            <dd class="font-medium">
              <span v-if="template.end_of_month">{{ t('recurring_invoice.end_of_month') }}</span>
              <span v-else-if="template.day_of_month">{{ t('recurring_invoice.day_n', { n: template.day_of_month }) }}</span>
              <span v-else>—</span>
            </dd>
          </div>
          <div class="flex justify-between">
            <dt class="text-neutral-500">{{ t('recurring_invoice.anchor_date') }}</dt>
            <dd class="font-medium">{{ formatDate(template.anchor_date) }}</dd>
          </div>
          <div v-if="template.end_date" class="flex justify-between">
            <dt class="text-neutral-500">{{ t('recurring_invoice.end_date') }}</dt>
          ...
          <div class="flex justify-between">
            <dt class="text-neutral-500">{{ t('recurring_invoice.payment_due_days') }}</dt>
            <dd class="font-medium">{{ template.payment_due_days }} {{ t('recurring_invoice.days') }}</dd>
          </div>
          <div class="flex justify-between">
            <dt class="text-neutral-500">{{ t('recurring_invoice.currency') }}</dt>
      ...
      <!-- Settings -->
      <div class="bg-white border border-neutral-200 rounded-lg p-5 shadow-sm">
        <h3 class="text-sm font-semibold uppercase tracking-wide text-neutral-500 mb-3">{{ t('recurring_invoice.settings') }}</h3>
        <dl class="space-y-1.5 text-sm">
          <div class="flex justify-between">
            <dt class="text-neutral-500">{{ t('recurring_invoice.payment_method') }}</dt>
            <dd class="font-medium">{{ t(`recurring_invoice.payment_method.${template.payment_method}`) }}</dd>
          </div>
          <div class="flex justify-between">
            <dt class="text-neutral-500">{{ t('recurring_invoice.language') }}</dt>
            <dd class="font-medium">{{ template.language === 'cs' ? 'Čeština' : 'English' }}</dd>
          </div>
          <div class="flex justify-between">
            <dt class="text-neutral-500">{{ t('recurring_invoice.reverse_charge') }}</dt>
            <dd class="font-medium">{{ template.reverse_charge ? '✓' : '—' }}</dd>
          </div>
          <div class="flex justify-between">
            <dt class="text-neutral-500">{{ t('recurring_invoice.auto_issue') }}</dt>
            <dd class="font-medium">{{ template.auto_issue ? '✓' : '—' }}</dd>
          </div>
          <div class="flex justify-between">
            <dt class="text-neutral-500">{{ t('recurring_invoice.auto_send_email') }}</dt>
            <dd class="font-medium">{{ template.auto_send_email ? '✓' : '—' }}</dd>
          </div>
          <div class="flex justify-between">
            <dt class="text-neutral-500">{{ t('recurring_invoice.increment_month') }}</dt>
      ...
      <!-- Upcoming runs -->
      <div class="bg-white border border-neutral-200 rounded-lg p-5 shadow-sm">
        <h3 class="text-sm font-semibold uppercase tracking-wide text-neutral-500 mb-3">{{ t('recurring_invoice.upcoming_runs') }}</h3>
        <div v-if="nextRuns.length === 0" class="text-sm text-neutral-500">{{ t('recurring_invoice.no_upcoming_runs') }}</div>
    ...
    <div class="bg-white border border-neutral-200 rounded-lg shadow-sm overflow-hidden mb-4">
      <div class="px-5 py-3 border-b border-neutral-200">
        <h3 class="text-sm font-semibold uppercase tracking-wide text-neutral-500">{{ t('recurring_invoice.items') }}</h3>
      </div>
        ...
        <table class="w-full text-sm">
          <thead class="bg-neutral-50 text-xs text-neutral-500 uppercase tracking-wide">
            <tr>
              ...
              <th class="px-4 py-2 text-right font-medium">{{ t('recurring_invoice.item_qty') }}</th>
              <th class="px-4 py-2 text-left font-medium">{{ t('recurring_invoice.item_unit') }}</th>
              <th class="px-4 py-2 text-right font-medium">{{ t('recurring_invoice.item_unit_price') }}</th>
              <th class="px-4 py-2 text-center font-medium">{{ t('recurring_invoice.item_vat') }}</th>
              <th class="px-4 py-2 text-right font-medium">{{ t('recurring_invoice.item_total') }}</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-neutral-100">
            <tr v-for="(item, i) in template.items" :key="item.id ?? i">
              ...
              <td class="px-4 py-2.5 text-center text-xs">
                ...
                <span v-else-if="item.vat_code === 'RC'" class="text-neutral-500">{{ t('recurring_invoice.reverse_charge') }}</span>
                <span v-else>0%</span>
              </td>
    ...
    <div class="bg-white border border-neutral-200 rounded-lg shadow-sm overflow-hidden">
      <div class="px-5 py-3 border-b border-neutral-200">
        <h3 class="text-sm font-semibold uppercase tracking-wide text-neutral-500">{{ t('recurring_invoice.generated_invoices') }}</h3>
      </div>
      <div v-if="generatedInvoices.length === 0" class="px-5 py-8 text-center text-neutral-500 text-sm">
        {{ t('recurring_invoice.no_generated_invoices') }}
      </div>
      <div v-else>
