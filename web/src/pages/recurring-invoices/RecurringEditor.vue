...
async function loadTemplate() {
  ...
  loading.value = true
  try {
    const tpl = await recurringInvoicesApi.get(templateId.value)
    form.value = {
...
async function submit() {
  if (!form.value.name.trim()) {
    toast.error(t('recurring_invoice.validation.name_required'))
    return
  }
  if (form.value.items.length === 0) {
    toast.error(t('recurring_invoice.validation.items_required'))
    return
  }
  submitting.value = true
  try {
    if (isEdit.value && templateId.value) {
      const updated = await recurringInvoicesApi.update(templateId.value, form.value)
      toast.success(t('common.saved'))
      savedId.value = updated.id
    } else {
      const created = await recurringInvoicesApi.create(form.value)
      toast.success(t('common.created'))
      savedId.value = created.id
      router.replace(`/recurring-invoices/${created.id}/edit`)
    }
  } catch (e: any) {
...
onMounted(async () => {
  const [vr, cur, un, cl] = await Promise.all([
...
<template>
  <div>
    <div class="flex items-center justify-between mb-4">
      <div>
        <RouterLink to="/recurring-invoices" class="text-sm text-neutral-600 hover:text-neutral-900">
          {{ t('recurring_invoice.back_to_list') }}
        </RouterLink>
        <h1 class="text-2xl font-semibold mt-1">
          {{ isEdit ? t('recurring_invoice.edit_title') : t('recurring_invoice.new_title') }}
        </h1>
      </div>
      <div class="flex items-center gap-2">
        <RouterLink
          v-if="savedId"
          :to="`/recurring-invoices/${savedId}`"
          ...
          {{ t('recurring_invoice.view_template') }}
        </RouterLink>
        <button
          @click="submit"
          :disabled="submitting"
    ...
    <div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-4">
      ...
      <div class="lg:col-span-2 space-y-4">
        ...
        <div class="bg-white border border-neutral-200 rounded-lg shadow-sm p-5">
          <h2 class="text-sm font-semibold uppercase tracking-wide text-neutral-500 mb-4">{{ t('recurring_invoice.basic_settings') }}</h2>
          <div class="space-y-4">
            <div>
              <label class="block text-sm font-medium text-neutral-700 mb-1">{{ t('recurring_invoice.name') }} *</label>
              <input v-model="form.name" type="text"
                ...
                :placeholder="t('recurring_invoice.name_placeholder')" />
            </div>

            <div>
              <label class="block text-sm font-medium text-neutral-700 mb-1">{{ t('recurring_invoice.client') }} *</label>
              <select v-model="form.client_id"
                class="w-full h-9 px-3 border border-neutral-300 rounded-md bg-white text-sm">
                <option :value="0">{{ t('recurring_invoice.select_client') }}</option>
                <option v-for="c in clients" :key="c.id" :value="c.id">
                  {{ c.company_name || c.first_name + ' ' + c.last_name }}
                  <span v-if="c.ic" class="text-neutral-400"> ({{ c.ic }})</span>
                </option>
              </select>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="block text-sm font-medium text-neutral-700 mb-1">{{ t('recurring_invoice.frequency') }}</label>
                <select v-model="form.frequency"
                  class="w-full h-9 px-3 border border-neutral-300 rounded-md bg-white text-sm">
                  <option value="monthly">{{ t('recurring_invoice.frequency.monthly') }}</option>
                  <option value="quarterly">{{ t('recurring_invoice.frequency.quarterly') }}</option>
                  <option value="semi_annually">{{ t('recurring_invoice.frequency.semi_annually') }}</option>
                  <option value="annually">{{ t('recurring_invoice.frequency.annually') }}</option>
                </select>
              </div>
              <div>
                <label class="block text-sm font-medium text-neutral-700 mb-1">{{ t('recurring_invoice.currency') }}</label>
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="block text-sm font-medium text-neutral-700 mb-1">{{ t('recurring_invoice.anchor_date') }} *</label>
                <input v-model="form.anchor_date" type="date"
                  class="w-full h-9 px-3 border border-neutral-300 rounded-md text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none" />
              </div>
              <div>
                <label class="block text-sm font-medium text-neutral-700 mb-1">{{ t('recurring_invoice.end_date') }}</label>
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="block text-sm font-medium text-neutral-700 mb-1">{{ t('recurring_invoice.day_of_month') }}</label>
                <div class="flex items-center gap-2">
              ...
              <div>
                <label class="block text-sm font-medium text-neutral-700 mb-1">{{ t('recurring_invoice.payment_due_days') }}</label>
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="block text-sm font-medium text-neutral-700 mb-1">{{ t('recurring_invoice.language') }}</label>
              ...
              <div>
                <label class="block text-sm font-medium text-neutral-700 mb-1">{{ t('recurring_invoice.payment_method') }}</label>
                <select v-model="form.payment_method"
                  class="w-full h-9 px-3 border border-neutral-300 rounded-md bg-white text-sm">
                  <option value="bank_transfer">{{ t('recurring_invoice.payment_method.bank_transfer') }}</option>
                  <option value="card">{{ t('recurring_invoice.payment_method.card') }}</option>
                  <option value="cash">{{ t('recurring_invoice.payment_method.cash') }}</option>
                  <option value="other">{{ t('recurring_invoice.payment_method.other') }}</option>
                </select>
              </div>
            </div>
            ...
            <label class="flex items-center gap-2 text-sm text-neutral-700">
              ...
              {{ t('recurring_invoice.reverse_charge') }}
            </label>
            <label class="flex items-center gap-2 text-sm text-neutral-700">
              <input v-model="form.increment_month_in_descriptions" type="checkbox" class="rounded border-neutral-300 text-primary-600" />
              {{ t('recurring_invoice.increment_month_in_descriptions') }}
            </label>
            <label class="flex items-center gap-2 text-sm text-neutral-700">
              <input v-model="form.auto_issue" type="checkbox" class="rounded border-neutral-300 text-primary-600" />
              {{ t('recurring_invoice.auto_issue') }}
            </label>
            <label class="flex items-center gap-2 text-sm text-neutral-700">
              <input v-model="form.auto_send_email" type="checkbox" class="rounded border-neutral-300 text-primary-600" />
              {{ t('recurring_invoice.auto_send_email') }}
            </label>
          </div>
        </div>
        ...
        <div class="bg-white border border-neutral-200 rounded-lg shadow-sm overflow-hidden">
          <div class="px-5 py-3 border-b border-neutral-200 flex items-center justify-between">
            <h2 class="text-sm font-semibold uppercase tracking-wide text-neutral-500">{{ t('recurring_invoice.items') }}</h2>
            ...
            <table class="w-full text-sm">
              <thead class="bg-neutral-50 text-xs text-neutral-500 uppercase tracking-wide">
                <tr>
                  ...
                  <th class="px-4 py-2 text-left font-medium">{{ t('recurring_invoice.item_description') }}</th>
                  <th class="px-4 py-2 text-right font-medium w-24">{{ t('recurring_invoice.item_qty') }}</th>
                  <th class="px-4 py-2 text-left font-medium w-20">{{ t('recurring_invoice.item_unit') }}</th>
                  <th class="px-4 py-2 text-right font-medium w-28">{{ t('recurring_invoice.item_unit_price') }}</th>
                  <th class="px-4 py-2 text-center font-medium w-28">{{ t('recurring_invoice.item_vat') }}</th>
                  <th class="px-4 py-2 text-right font-medium w-28">{{ t('recurring_invoice.item_total') }}</th>
                ...
                <tr v-for="(item, i) in form.items" :key="i">
                  ...
                  <td class="px-4 py-2">
                    <input v-model="item.description" type="text"
                      ...
                      :placeholder="t('recurring_invoice.item_description_placeholder')" />
                  </td>
                  <td class="px-4 py-2">
                  ...
                  <td class="px-4 py-2">
                    <select v-model="item.vat_rate_id"
                      ...
                      <option v-for="vr in vatRates" :key="vr.id" :value="vr.id">
                        {{ Number(vr.rate_percent) > 0 ? `${vr.rate_percent}%` : (vr.is_reverse_charge ? t('recurring_invoice.reverse_charge') : '0%') }}
                      </option>
                    </select>
                  </td>
              ...
              <tfoot class="bg-neutral-50 border-t border-neutral-200">
                <tr>
                  <td colspan="6" class="px-4 py-2 text-right text-sm font-medium text-neutral-600">{{ t('recurring_invoice.total_without_vat') }}:</td>
                ...
                <tr>
                  <td colspan="6" class="px-4 py-2 text-right text-sm font-semibold text-neutral-900">{{ t('recurring_invoice.total_with_vat') }}:</td>
        ...
        <!-- Notes -->
        <div class="bg-white border border-neutral-200 rounded-lg shadow-sm p-5">
          <h2 class="text-sm font-semibold uppercase tracking-wide text-neutral-500 mb-4">{{ t('recurring_invoice.notes') }}</h2>
          <div class="space-y-4">
            <div>
              <label class="block text-sm font-medium text-neutral-700 mb-1">{{ t('recurring_invoice.note_above_items') }}</label>
            ...
            <div>
              <label class="block text-sm font-medium text-neutral-700 mb-1">{{ t('recurring_invoice.note_below_items') }}</label>
      ...
      <div class="space-y-4">
        <!-- Summary -->
        <div class="bg-white border border-neutral-200 rounded-lg shadow-sm p-5">
          <h2 class="text-sm font-semibold uppercase tracking-wide text-neutral-500 mb-3">{{ t('recurring_invoice.summary') }}</h2>
          <dl class="space-y-2 text-sm">
            <div class="flex justify-between">
              <dt class="text-neutral-500">{{ t('recurring_invoice.status') }}</dt>
              <dd class="font-medium">{{ isEdit ? '—' : t('recurring_invoice.status.active') }}</dd>
            </div>
            <div class="flex justify-between">
              <dt class="text-neutral-500">{{ t('recurring_invoice.next_run') }}</dt>
              <dd class="font-medium">{{ form.anchor_date }}</dd>
            </div>
            <div class="flex justify-between">
              <dt class="text-neutral-500">{{ t('recurring_invoice.items_count') }}</dt>
              <dd class="font-medium">{{ form.items.length }}</dd>
            </div>
            <div v-if="selectedClient">
              <dt class="text-neutral-500">{{ t('recurring_invoice.client') }}</dt>
              <dd class="font-medium">{{ selectedClient.company_name || (selectedClient.first_name + ' ' + selectedClient.last_name) }}</dd>
            </div>
          </dl>
        </div>
        ...
        <!-- Quick info -->
        <div class="bg-white border border-neutral-200 rounded-lg shadow-sm p-5">
          <h2 class="text-sm font-semibold uppercase tracking-wide text-neutral-500 mb-3">{{ t('recurring_invoice.how_it_works') }}</h2>
          <p class="text-xs text-neutral-600 leading-relaxed">
            {{ t('recurring_invoice.how_it_works_description') }}
          </p>
        </div>
      </div>
    </div>
