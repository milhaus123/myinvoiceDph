...
/**
 * Generuje nákupní fakturu ze šablony pravidelného nákupu.
 *
 * Kroky:
 *   1. Vytvoří draft (klon šablony — supplier, project, currency, language,
 *      payment_method, reverse_charge, notes; položky se zkopírují).
 *   2. Pokud auto_issue=true: přechod draft → received (status).
 *   3. Posune next_run_date na šabloně (PeriodicityCalculator) a updatuje
 *      last_run_date; pokud nové next > end_date, status='expired'.
 */
final class RecurringPurchaseInvoiceGenerator
{
    public function __construct(
        private readonly Connection $db,
        private readonly PurchaseInvoiceTemplateRepository $templates,
    ...
    public function generate(int $templateId, ?string $forcedIssueDate = null, ?int $userId = null): array
    {
        ...
        if (!empty($template['auto_issue'])) {
            $this->invoices->markStatus($invoiceId, 'received');
            $issued = true;
        }
    ...
    private function createInvoiceFromTemplate(array $template, string $issueDate, string $dueDate, ?int $userId): int
    {
        ...
        $stmt->execute([
            ...
            $issueDate,
            $issueDate, // tax_date = issue_date pro nákupní faktury
            $dueDate,
            $issueDate, // received_at
