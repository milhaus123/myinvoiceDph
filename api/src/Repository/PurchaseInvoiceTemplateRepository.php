...
/**
 * CRUD pro recurring_purchase_invoice_templates + items.
 *
 * Šablona je strukturálně blízká nákupní faktuře (supplier, project, currency, items, ...),
 * ale má navíc periodicitu a chování cronu (auto_issue).
 *
 * Listing pro cron je v findDue() — vrátí jen aktivní šablony, kde
 * next_run_date <= today.
 */
final class PurchaseInvoiceTemplateRepository
{
    ...
    public function find(int $id): ?array
    {
        $stmt = $this->db->pdo()->prepare(
            'SELECT t.*,
                    ...
                    c.main_email AS supplier_main_email,
                    c.ic AS supplier_ic,
                    c.dic AS supplier_dic,
               ...
               FROM recurring_purchase_invoice_templates t
               JOIN clients c ON c.id = t.supplier_id
    ...
    public function itemsFor(int $templateId): array
    {
        $stmt = $this->db->pdo()->prepare(
            'SELECT i.id, i.template_id, i.description, i.quantity, i.unit,
               ...
               FROM recurring_purchase_invoice_template_items i
    ...
    public function list(array $filters = []): array
    {
        ...
        $sql = "SELECT t.id, t.supplier_id, t.project_id, t.name,
                       t.frequency, t.day_of_month, t.end_of_month,
                       t.anchor_date, t.end_date, t.next_run_date, t.last_run_date,
                       t.currency_id, t.language, t.payment_method,
                       t.payment_due_days, t.reverse_charge,
                       t.auto_issue, t.status,
                       t.created_at, t.updated_at,
                       c.company_name AS supplier_company_name,
                       p.name AS project_name,
                       cur.code AS currency,
                       (SELECT COUNT(*) FROM purchase_invoices pi WHERE pi.recurring_template_id = t.id) AS invoices_generated_count
                  FROM recurring_purchase_invoice_templates t
                  JOIN clients c ON c.id = t.supplier_id
             LEFT JOIN projects p ON p.id = t.project_id
                  JOIN currencies cur ON cur.id = t.currency_id
    ...
    /**
     * Načte šablony, které mají dnes nebo dříve `next_run_date` a jsou aktivní.
     */
    ...
    public function create(array $data, int $userId): int
    {
        ...
        $sql = 'INSERT INTO recurring_purchase_invoice_templates
            (supplier_id, project_id, name,
             ...
             increment_month_in_descriptions, auto_issue, status, created_by)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
    ...
    {
        $sql = 'UPDATE recurring_purchase_invoice_templates SET
                supplier_id = ?, project_id = ?, name = ?,
                frequency = ?, day_of_month = ?, end_of_month = ?,
                anchor_date = ?, end_date = ?,
                currency_id = ?, language = ?, payment_method = ?,
                reverse_charge = ?, payment_due_days = ?,
                note_above_items = ?, note_below_items = ?,
                increment_month_in_descriptions = ?, auto_issue = ?
              WHERE id = ?';
    ...
    {
        $pdo = $this->db->pdo();
        $pdo->prepare('DELETE FROM recurring_purchase_invoice_template_items WHERE template_id = ?')
            ->execute([$templateId]);

        $stmt = $pdo->prepare(
            'INSERT INTO recurring_purchase_invoice_template_items
    ...
    {
        $this->db->pdo()->prepare(
            'UPDATE recurring_purchase_invoice_templates
    ...
    {
        $this->db->pdo()->prepare(
            'UPDATE recurring_purchase_invoice_templates SET status = ? WHERE id = ?'
        )->execute([$status, $id]);
    }
    ...
    public function delete(int $id): void
    {
        ...
        $this->db->pdo()->prepare('DELETE FROM recurring_purchase_invoice_templates WHERE id = ?')
            ->execute([$id]);
    }
    ...
    private function cast(array $row): array
    {
        ...
        foreach (['end_of_month', 'reverse_charge', 'increment_month_in_descriptions', 'auto_issue'] as $f) {
