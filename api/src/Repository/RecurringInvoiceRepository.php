 ...
 *
 * Šablona je strukturálně blízká vydané faktuře (client, project, currency, items, ...),
...
final class RecurringInvoiceRepository
{
    ...
    public function list(array $filters = []): array
    {
        ...
        if (!empty($filters['client_ids']) && is_array($filters['client_ids'])) {
            $placeholders = implode(',', array_fill(0, count($filters['client_ids']), '?'));
            $where[] = "t.client_id IN ($placeholders)";
            foreach ($filters['client_ids'] as $cid) $params[] = (int) $cid;
        }
        ...
        $sql = "SELECT t.id, t.client_id, t.project_id, t.name,
                       ...
                       (SELECT COUNT(*) FROM invoices i WHERE i.recurring_template_id = t.id) AS invoices_generated_count
                  FROM recurring_invoice_templates t
                  JOIN clients c ON c.id = t.client_id
             LEFT JOIN projects p ON p.id = t.project_id
                  JOIN currencies cur ON cur.id = t.currency_id
